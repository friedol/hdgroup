...
class LoanController extends Controller
{
    ...
    private function getLoanData($request, $getAll = false)
    {
        $paymentsSub = DB::table('payments')
            ->select('unique_id', DB::raw('SUM(amount_paid) as total_paid'))
            ->groupBy('unique_id');
        ...
        $query = Loan::select(
            ...
            DB::raw('COALESCE(MAX(p.total_paid), 0) as total_paid'),
            DB::raw('SUM(loans.total_amount) - COALESCE(MAX(p.total_paid), 0) as balance'),
            'loans.payment_date',
            'loans.created_at',
            DB::raw('MAX(loans.staff_recommeded) as notes')
        )
            ->leftJoinSub($paymentsSub, 'p', 'loans.unique_id', '=', 'p.unique_id')
        ...
        if ($status === 'all') {
            ...
            $query->havingRaw('COALESCE(MAX(p.total_paid), 0) >= SUM(loans.total_amount)');
        } elseif ($status === 'unpaid') {
            $query->havingRaw('COALESCE(MAX(p.total_paid), 0) = 0');
        } elseif ($status === 'uncomplete') {
            $query->havingRaw('COALESCE(MAX(p.total_paid), 0) > 0')
                ->havingRaw('COALESCE(MAX(p.total_paid), 0) < SUM(loans.total_amount)');
    ...
    public function create()
    {
        ...
        return Inertia::render('Finance/Loans/Create', compact('products', 'users'));
    }
    ...
    public function index(Request $request)
    {
        ...
        $summary = [
            'total_debt' => (float) $allLoans->sum(function ($l) {
                return max(0, $l->total_amount - $l->total_paid);
            }),
            ...
            'active_loans' => $allLoans->filter(function ($l) {
                return $l->total_paid < $l->total_amount;
            })->count(),
        ...
        return Inertia::render('Finance/Loans/Index', compact('products', 'loans', 'users', 'summary', 'chartData', 'filters'));
    }
    ...
    public function reports(Request $request)
    {
        ...
        return Inertia::render('Admin/Loans/Reports', $d);
    }
    ...
    public function show($unique_id)
    {
        ...
        foreach ($loans as $loan) {
            if (! $loan->linked_sku) {
                $fallback = ProductManagement::where('product_name', $loan->product_name)->first();
        ...
        return Inertia::render('Finance/Loans/Show', $d);
    }
    ...
    public function edit($unique_id)
    {
        ...
        return Inertia::render('Finance/Loans/Edit', $d);
    }
