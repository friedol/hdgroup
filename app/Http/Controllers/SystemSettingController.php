...
class SystemSettingController extends Controller
{
    ...
    public function index()
    {
        ...
        $branches = \App\Models\Branch::all()->map(function($b) {
            return [
                'id'          => $b->id,
                'name'        => $b->name,
                ...
                'users'       => \App\Models\User::where('branch_id', $b->id)->count()
            ];
        });

        $stores = \App\Models\Store::with('branches')->get()->map(function($s) {
            return [
                'id' => $s->id,
                'name' => $s->store_name,
        ...
        $heroSlides = \App\Models\HeroSlide::orderBy('is_ad')->orderBy('sort_order')->get();
    ...
    {
        if (str_contains($key, 'module_')) return 'modules';
        if (str_contains($key, 'business_whatsapp')) return 'contact';
        if (str_contains($key, 'business_') || str_contains($key, 'system_') || str_contains($key, 'color') || str_contains($key, 'font') || str_contains($key, 'branding_')) return 'branding';
        if (str_contains($key, 'tax_') || str_contains($key, 'currency_') || str_contains($key, 'bank_')) return 'financial';
        if (str_contains($key, 'stock_') || str_contains($key, 'inventory_') || str_contains($key, 'allow_negative')) return 'inventory';
        if (str_contains($key, 'sms_')) return 'sms';
        if (str_contains($key, 'mail_')) return 'email';
        if (str_contains($key, 'social_')) return 'social';
        if (str_contains($key, 'logistics_')) return 'logistics';
        if (str_contains($key, 'date_') || str_contains($key, 'timezone') || str_contains($key, 'language')) return 'preferences';
        return 'general';
    }
    ...
    public function testEmail(Request $request)
    {
        try {
            ...
            \Illuminate\Support\Facades\Mail::raw('This is a test email to verify your HD Group SMTP settings.', function($message) use ($request) {
