...
interface PopupAdFormProps {
  ...
  existingSlide?: PopupAd | null;
}

export default function PopupAdForm({ branches, onSuccess, editingSlideId, existingSlide }: PopupAdFormProps) {
  ...
  const submitOptions = {
    forceFormData: true,
    onSuccess: () => {
      toast.success(editingSlideId ? "Popup ad updated successfully" : "Popup ad created successfully");
      onSuccess();
    },
    onError: (formErrors: Record<string, string>) => {
      const firstError = Object.values(formErrors)[0];
      toast.error(firstError || "Failed to save popup ad");
    },
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingSlideId) {
      put(`/settings/popup-ads/${editingSlideId}`, submitOptions);
      return;
    }

    post("/settings/popup-ads", submitOptions);
  };
  ...
  return (
    <div className="space-y-6">
      <Card className="border-slate-200 shadow-none">
        <CardHeader className="pb-4 border-b border-slate-100 mb-6 px-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-sm font-semibold">Popup image</CardTitle>
            </div>
          </div>
        </CardHeader>
        <CardContent className="space-y-6 px-6 pb-6">
          <div className="border border-dashed border-slate-200 rounded-xl p-6 flex flex-col items-center justify-center gap-4 bg-slate-50/50">
            <div className="w-full aspect-square rounded overflow-hidden shadow-sm flex items-center justify-center bg-slate-100 max-w-sm mx-auto">
              {preview ? (
                <img src={preview} alt="Preview" className="w-full h-full object-cover" />
              ) : (
                <div className="text-center space-y-2">
                  <ImageIcon size={32} className="mx-auto text-slate-300" />
                  <p className="text-xs text-slate-400 font-medium">Select a square image</p>
                </div>
              )}
            </div>
      ...
      <Card className="border-slate-200 shadow-none">
        <CardHeader className="pb-4 border-b border-slate-100 mb-6 px-6">
          <div>
            <CardTitle className="text-sm font-semibold">Popup details</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="space-y-6 px-6 pb-6">
        ...
        <CardFooter className="px-6 py-4 bg-slate-50/50 border-t border-slate-100 flex justify-end gap-3">
          ...
          <Button
            ...
            disabled={processing}
          >
            {processing ? "Saving..." : editingSlideId ? "Update popup" : "Create popup"}
          </Button>
        </CardFooter>
      </Card>
    </div>
